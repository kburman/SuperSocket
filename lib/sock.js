...
function parser (socket, option) {
	if (socket == undefined || !(socket instanceof Socket)) {
...
/*
	Since we cant socket object to parser
	we need some one can which can handle it
*/

function parserForServer (socket) {
	if (socket == undefined || (socket instanceof Socket)) {
		throw new Error('Socket cant be undefined')
	}
...
// inherit the event emitter
util.inherits(parserForServer, EventEmitter)



parserForServer.prototype._handleData = function(chunk) {
	var str = chunk.toString()
	if (str.indexOf(EOD) > 0) {
		// so we got our header
		var split = str.split(EOD)
...
parserForServer.prototype.send = function(msg) {
	this._socket.write(this._translator.encode(msg) + EOD)
}

parserForServer.prototype.getSocket = function() {
	return this._socket
}
...
function server (port, host, option) {
	this._host = host
	this._port = port
	this._option = option
	this._client = []
...
server.prototype._handleConnection = function(client) {
	var obj = new parserForServer(client)
	var self = this
	this._client.push(obj)

	// for back track
	client.obj = obj

	// now handle some error
	client.on('error', function (err) {
		var i
		if ((i = self._client.indexOf(this.obj)) > 0) {
			self._client.splice(i, 1)
		} else {
			throw new Error('For some reason socket is not in our list : ')
		}
	})

	client.on('end', function () {
		var i
		if ((i = self._client.indexOf(this)) > 0) {
			self._client.splice(i, 1)
		} else {
			throw new Error('For some reason socket is not in our list')
		}
	...
	obj.on('msg', function (msg) {
		self.emit('msg', this, msg)
	})
...
server.prototype.sendAll = function(msg) {
	for (var i = this._client.length - 1; i >= 0; i--) {
		this._client[i].send(msg)
	}
}

server.prototype.sendAllExcept = function(msg, client) {
	if (!(client instanceof parserForServer)) {
		throw new Error('client should of type parserForServer')
	}

	for (var i = this._client.length - 1; i >= 0; i--) {
		if (client != this._client[i]) {
			this._client[i].send(msg)
		}
	}
}
